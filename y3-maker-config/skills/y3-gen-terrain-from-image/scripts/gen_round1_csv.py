...
def build_height_grid_from_config(continent_map, water_mask, height_config):
    """根据大陆高度配置构建每格高度矩阵。

    Args:
        continent_map: (H, W) int32
        water_mask:    (H, W) bool
        height_config: dict[int, int]，大陆ID → 高度值

    Returns:
        height_grid: (H, W) int32，水域格为 0（水体高度由引擎控制）
    """
    H, W = continent_map.shape
...
def load_slope_cells(boundary_report_path, slope_decisions_path):
    ...
    for d in decisions_list:
        ...
        if d.get("traversable", False):
            traversable_ids.add(bid)
            if "cells_override" in d and d["cells_override"]:
                override_cells[bid] = d["cells_override"]
...
def generate_csvs(water_mask, continent_map, full_texture_config, cliff_config, default_texture,
    ...
    Args:
        ...
        default_texture: int
        output_dir: str
        height_grid: (H, W) int32 或 None，每格高度值；None 时全部为 0
        slope_cells: set of (x, z) 或 None，需标记为 slope 的格子
        water_type_grid: (H, W) int8 或 None，水体类型
                         (0=陆地, 1=deep_water, 2=shallow_water, 3=plain_water)
                         None 时所有水域格子默认为 deep_water
    """
    H, W = water_mask.shape
    if slope_cells is None:
        slope_cells = set()
    ...
    # 水体类型名称映射
    WATER_TYPE_NAMES = {1: "deep_water", 2: "shallow_water", 3: "plain_water"}
    ...
    with open(terrain_path, "w", newline="") as tf, \
        ...
        for z in range(H):
            ...
            for x in range(W):
                ...
                if water_mask[z, x]:
                    # 从 water_type_grid 读取具体水体类型，默认深水
                    if water_type_grid is not None:
                        wtype = int(water_type_grid[z, x])
                        water_name = WATER_TYPE_NAMES.get(wtype, "deep_water")
                    else:
                        water_name = "deep_water"
                    t_row.append(f"{water_name},0,{ctid}")
                    x_row.append(0)
                    water_count += 1
                    if water_name == "deep_water":    deep_count += 1
                    elif water_name == "shallow_water": shallow_count += 1
                    elif water_name == "plain_water":   plain_count += 1
                else:
    ...
    # 打印水体类型分布
    if water_type_grid is not None and (shallow_count + plain_count) > 0:
        print(f"  水体类型: 深水={deep_count}, 浅水={shallow_count}, 平面水={plain_count}")
    # 打印高度分布
    if any(h > 0 for h in height_counts):
        print("  高度分布:")
        for h in sorted(height_counts):
            print(f"    h={h}: {height_counts[h]} 格")
    if slope_count:
        print(f"  斜坡格: {slope_count}")
...
def main():
    ...
    parser.add_argument("--boundary-report", default=None,
                        help="height_boundary_report.json 路径（与 --slope-decisions 联合使用）")
    parser.add_argument("--slope-decisions", default=None,
                        help="slope_decision.json 路径，AI 斜坡规划决策文件")
    parser.add_argument("--water-type-grid", default=None,
                        help="water_type_grid.npy 路径（cv_water_classify.py 输出）；"
                             "提供时按格子水体类型写入 CSV，否则全部为 deep_water")
    ...
    if args.group_height_config:
        ...
        try:
            ...
            group_height = {int(k): int(v) for k, v in raw_h.items()}
        except (json.JSONDecodeError, ValueError) as e:
            print(f"[ERROR] group-height-config JSON 解析失败: {e}")
            sys.exit(1)

        # 将组高度展开为大陆高度（复用 texture 分组结果）
        if not args.continent_summary:
            auto_path = os.path.join(args.output_dir, "continent_summary.json")
            if os.path.exists(auto_path):
                args.continent_summary = auto_path
        if args.continent_summary:
            groups, _ = group_continents_by_color(args.continent_summary, args.color_threshold)
            height_config_per_continent = expand_group_config(groups, [], group_height)
        else:
            # 没有 continent_summary 时，直接按大陆 ID 使用
            height_config_per_continent = group_height

        print(f"  高度配置(按大陆): {height_config_per_continent}")
        height_grid = build_height_grid_from_config(continent_map, water_mask, height_config_per_continent)

        # 打印高度分布预览
        unique_heights = sorted(set(int(v) for v in np.unique(height_grid) if v >= 0))
    ...
    if args.water_type_grid:
        if not os.path.exists(args.water_type_grid):
            print(f"[WARN] water-type-grid 不存在: {args.water_type_grid}，将使用默认深水")
        else:
            water_type_grid = np.load(args.water_type_grid).astype(np.int8)
            print(f"\n[Step 2d] 加载水体类型网格: {args.water_type_grid}")
            for code, name in [(1,"深水"),(2,"浅水"),(3,"平面水")]:
                cnt = int(np.sum(water_type_grid == code))
                if cnt:
                    print(f"  {name}: {cnt} 格")
