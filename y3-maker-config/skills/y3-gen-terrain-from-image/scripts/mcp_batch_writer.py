...
def compute_stats(terrain_grid, texture_grid):
    ...
    print(f"   plain_water:   {stats['plain_water']:>6} 格")
...
def run_pass_water(mcp, terrain_grid, dry_run=False):
    ...
    for z in range(rows):
        for x in range(cols):
            t, h, cid = terrain_grid[z][x]
            if t == "plain_water":
                pw_cells.append({"x": x, "z": z})

    if pw_cells:
        print(f"   📍 平面水: {len(pw_cells)} 格")
        if not dry_run:
            call_batched(mcp, "terrain_set_plain_water_block", pw_cells, {}, "Pass 3 PlainWater")
    else:
        print("   ⏭️ 无平面水格子")
    ...
    if pass_id == "crack":
        ...
        for z in range(rows):
            for x in range(cols):
                t, h, cid = terrain_grid[z][x]
                if t == "plain_water":
                    all_cells.append({"x": x, "z": z, "_tool": "terrain_set_plain_water_block"})
        if not all_cells:
            return None
        dw_cid = max(dw_cid_counter, key=dw_cid_counter.get) if dw_cid_counter else 0
    ...
    for pass_idx, pass_id in enumerate(PASS_ORDER):
        ...
        while batch_start < num_batches and batches_executed < max_batches:
            ...
            if tool_name == "_water_mixed":
                ...
                dw_batch = [{"x": c["x"], "z": c["z"]} for c in batch if c.get("_tool") == "terrain_set_deep_water_block"]
                sw_batch = [{"x": c["x"], "z": c["z"]} for c in batch if c.get("_tool") == "terrain_set_shallow_water_block"]
                pw_batch = [{"x": c["x"], "z": c["z"]} for c in batch if c.get("_tool") == "terrain_set_plain_water_block"]
                if dw_batch:
                    args = {"cells": dw_batch}
                    args.update(extra_args)
                ...
                if pw_batch:
                    args = {"cells": pw_batch}
                    print(f"   ⏳ {batch_label}: {len(pw_batch)} 平面水格发送中...")
                    call_with_retry(mcp, "terrain_set_plain_water_block", args, batch_label)
            else:
                args = {"cells": batch}
                args.update(extra_args)
